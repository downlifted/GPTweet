...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    (async () => {
        if (request.type === "generate_tweet") {
            ...
            const prompt = `Write a ${props.type} tweet${props.topic ? ` about ${props.topic}` : ""}${props.replyTo ? ` in reply to a tweet "${props.replyTo}"` : ""}. Use locale "${props.locale}". Keep it short and don't use hashtags.`;
